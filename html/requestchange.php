...
$sql .= "WHERE request_on > 'today'::timestamp - '1 week'::interval GROUP BY system_code;";
add_system_data( $sql, 'new_in_week' );

$sql = "SELECT system_code AS key, count(*) AS data FROM request_status INNER JOIN request USING ( request_id ) ";
...
foreach( $systems AS $scode => $sys ) {
  $class = " class=row" . $i++ % 2;
  $url1 = "/requestrank.php?system_code=".urlencode($scode)."&inactive=0&status[N]=1&status[R]=1&status[H]=1&status[C]=1&status[I]=1&status[L]=1&status[T]=1&status[Q]=1&status[A]=1&status[D]=1&status[S]=1&status[P]=1&status[Z]=1";
  $url2 = "/requestlist.php?style=stripped&format=brief&system_code=".urlencode($scode)."&inactive=0&incstat[N]=1&incstat[R]=1&incstat[H]=1&incstat[C]=1&incstat[I]=1&incstat[L]=1&incstat[T]=1&incstat[Q]=1&incstat[A]=1&incstat[D]=1&incstat[S]=1&incstat[P]=1&incstat[Z]=1";
  echo "<tr>\n";
